...
public class Main {
        ...
        do {
            switch (animalChoiceMenu(keyboard)) {
                ...
                case 4:
                    ...
                    break;
                case 5:
                    System.out.println("Retrieving animals from file...");
                    readObjectsFromFile();
                    break;
                default:
    ...
    public static void writeObjectsToFile(Tiger tigerObject, Penguin penguinObject, Dolphin dolphinObject) {
        ...
        try(ObjectOutputStream writer = new ObjectOutputStream(new FileOutputStream(tigerFile))){
            writer.writeObject(tigerObject);
            System.out.println("Tiger saved to file.");
        }catch (IOException e){
            e.printStackTrace();
        }
        try(ObjectOutputStream writer = new ObjectOutputStream(new FileOutputStream(penguinFile))){
            writer.writeObject(penguinObject);
            System.out.println("Penguin saved to file.");
        }catch (IOException e){
            e.printStackTrace();
        }
        try(ObjectOutputStream writer=new ObjectOutputStream(new FileOutputStream(dolphinFile))){
            writer.writeObject(dolphinObject);
    ...
    public static void readObjectsFromFile(){
        ...
        try (FileInputStream fileIn = new FileInputStream(tigerFile);ObjectInputStream in=new ObjectInputStream(fileIn)){
            tigerObject=(Tiger)in.readObject();
            System.out.println("Tiger data retrieved from file: " + tigerObject.toString());
        }catch (IOException | ClassNotFoundException e){
            e.printStackTrace();
        }
        try (FileInputStream fileIn=new FileInputStream(penguinFile);ObjectInputStream in=new ObjectInputStream(fileIn)){
            penguinObject=(Penguin) in.readObject();
            System.out.println("Penguin data retrieved from file: " + penguinObject.toString());
        }catch (IOException | ClassNotFoundException e){
            e.printStackTrace();
        }
        try(FileInputStream fileIn=new FileInputStream(dolphinFile);ObjectInputStream in=new ObjectInputStream(fileIn)){
            dolphinObject=(Dolphin) in.readObject();
            System.out.println("Dolphin data retrieved from file: " + dolphinObject.toString());
        }catch (IOException | ClassNotFoundException e){
            e.printStackTrace();
        }
    }
