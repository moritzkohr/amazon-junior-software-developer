...
public class StudentInfoSystem {
    ...
    public List<Subject> sortSubjectsByName() {
        ...
        int regCount = subjectsByName.size();
        boolean swapped;
        for (int i = 0; i < regCount; i++) {
            swapped = false;
            for (int j = 0; j < regCount - 1 - i; j++) {
                Subject currentSub = (Subject) subjectsByName.get(j);
                Subject nextSub = (Subject) subjectsByName.get(j + 1);
                if (currentSub.name.compareTo(nextSub.name) > 0) {
                    subjectsByName.set(j, nextSub);
                    subjectsByName.set(j + 1, currentSub);
                    swapped = true;
                }
            }
            if (!swapped) {
    ...
    public List<Student> sortStudentsByName() {
        ...
        for (int unsortedIndx = 1; unsortedIndx < studentCount; unsortedIndx++) {
            Student unsortedStudent = (Student) students.get(unsortedIndx);
            int sortedIndx = unsortedIndx - 1;
            while (sortedIndx >= 0) {
                Student sortedStudent = (Student) students.get(sortedIndx);
                if (unsortedStudent.name.compareTo(sortedStudent.name) < 0) {
                    students.set(sortedIndx + 1, sortedStudent);
                    sortedIndx--;
                } else {
                    break;
                }
            }
            students.set(sortedIndx + 1, unsortedStudent);
        }
        return students;
    }
    ...
    public Student findStudent(Integer studentNumber) {
        ...
        while (left <= right) {
            ...
            int comparitor = students.get(middle).studentNumber.compareTo(studentNumber);
            if (comparitor == 0) {
